class Character 
  constructor: (msg, robot) ->
    ...
    msg.send "You are now a#{if (character.race in ['Elf', 'Orc']) then 'n' else ''} #{character.race} #{character.class} [Strength: #{character.strength}, Dexterity: #{character.dexterity}, Intelligence: #{character.intelligence}, Wisdom: #{character.wisdom}, Charisma: #{character.charisma}]"
...
module.exports = (robot) ->
  robot.hear /who am i?/i, (msg) ->
    ...
    msg.send "You are #{msg.message.user.id} a#{if (character.race in ['Elf', 'Orc']) then 'n' else ''} #{character.race} #{character.class} [String: #{character.strength}, Dexterity: #{character.dexterity}, Inteligence: #{character.intelligence}, Wisom: #{character.wisdom}, Charisma: #{character.charisma}]."
